...
def create_contract_buildpro():
    ...
    story.append(Paragraph("Payment Terms: Monthly invoicing based on completion milestones", styles['Normal']))
    story.append(Spacer(1, 0.2*inch))
    ...
    story.append(Paragraph(f"Signature Date: {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
...
def create_contract_techsupply():
    ...
    story.append(Paragraph("• Supply of 100x High-performance servers (2U rack format)", styles['Normal']))
    story.append(Paragraph("• Network infrastructure equipment (switches, routers, firewalls)", styles['Normal']))
    ...
    story.append(Paragraph("Payment: 30% upon order, 50% upon delivery, 20% upon installation completion", styles['Normal']))
    story.append(Spacer(1, 0.3*inch))
    
    story.append(Paragraph(f"Agreement Date: {datetime.now().strftime('%B %d, %Y')}", styles['Normal']))
...
def create_pricelist_mediequip():
    ...
    story.append(Paragraph("• All prices in USD and subject to 2% increase on January 1, 2025", styles['Normal']))
    story.append(Paragraph("• Bulk discounts apply to orders of specified quantities", styles['Normal']))
    story.append(Paragraph("• Delivery: 2-4 weeks from order confirmation", styles['Normal']))
    story.append(Paragraph("• Installation and training available at additional cost", styles['Normal']))
...
def create_pricelist_officemart():
    ...
    story.append(Paragraph("Contact: sales@officemart.com | Phone: 1-800-OFFICE1", styles['Normal']))
    story.append(Spacer(1, 0.3*inch))
    
    # Create pricing table
    data = [
        ['SKU', 'Product Name', 'Unit Price', 'Box Price', 'MOQ'],
        ['OM-PAPER-001', 'Copy Paper (8.5x11, 20lb)', '$5.99', '$55.00 (10 reams)', '100 reams'],
        ['OM-PEN-002', 'Ballpoint Pens (Blue, box of 50)', '$12.50', 'N/A', '500 units'],
        ...
        ['OM-FOLDER-006', 'Manila Folders (Letter, 100ct)', '$8.75', '$82.00 (10 box)', '100 units'],
        ['OM-LABEL-007', 'Mailing Labels (100 sheets)', '$4.95', '$44.00 (10 pack)', '500 sheets'],
    ...
    story.append(Paragraph("• Volume discounts: 10% off for orders over $1,000, 15% off for over $5,000", styles['Normal']))
    story.append(Paragraph("• Free shipping on orders over $500 (continental US)", styles['Normal']))
